		...
		if(attachs != null){
			for(int i = 0;i < attachs.length ;i++){
				...
				if(!attach.isEmpty()){
					...
					String prefix=FilenameUtils.getExtension(oldFileName);//原文件后�?     
					int filesize = 500000;
			        if(attach.getSize() >  filesize){//上传大小不得超过 500k
			        	request.setAttribute(errorInfo, " * 上传大小不得超过 500k");
			        	flag = false;
		            }else if(prefix.equalsIgnoreCase("jpg") || prefix.equalsIgnoreCase("png") 
		            		|| prefix.equalsIgnoreCase("jpeg") || prefix.equalsIgnoreCase("pneg")){//上传图片格式不正�?
		...
		for(int i = 0;i < attachs.length ;i++){
			...
			if(!attach.isEmpty()){
		        ...
		        if(attach.getSize() >  filesize){//上传大小不得超过 500k
	            	...
	            	request.setAttribute(errorInfo, " * 上传图片格式不正�?");
	            	flag = false;
	            }
			}
		}
